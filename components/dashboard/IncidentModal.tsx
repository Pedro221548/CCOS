...
interface IncidentModalProps {
    selectedDeviceForInfo: Camera | AccessPoint | null;
    closeInfoModal: () => void;
    localTicket: string;
...
const IncidentModal: React.FC<IncidentModalProps> = ({
    ...
    const isCamera = selectedDeviceForInfo && 'channelType' in selectedDeviceForInfo;
    const isAlarm = isCamera && (selectedDeviceForInfo as Camera).channelType === 'alarm';
    const isAccessPoint = selectedDeviceForInfo && !('channelType' in selectedDeviceForInfo);
                    ...
                    >
                        <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950/50">
                            <div className="flex items-center gap-3">
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isAlarm ? 'bg-amber-500/10 text-amber-500' : isAccessPoint ? 'bg-indigo-500/10 text-indigo-500' : 'bg-blue-500/10 text-blue-500'}`}>
                                    <AlertTriangle size={20} />
                                </div>
                                <div>
                                    <h3 className="text-lg font-black text-slate-800 dark:text-white uppercase tracking-tight">{selectedDeviceForInfo.name}</h3>
                                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                                        {isAlarm ? 'Canal de Alarme' : isAccessPoint ? 'Ponto de Acesso' : 'Câmera de Vídeo'}
                                    </p>
                                </div>
                            </div>
                        ...
                        <div className="p-6 space-y-6">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-slate-50 dark:bg-slate-950/50 p-4 rounded-2xl border border-slate-200 dark:border-slate-800">
                                    ...
                                    <p className="text-sm font-bold text-slate-800 dark:text-slate-200 uppercase">{selectedDeviceForInfo.location}</p>
                                </div>
                                <div className="bg-slate-50 dark:bg-slate-950/50 p-4 rounded-2xl border border-slate-200 dark:border-slate-800">
                                    <div className="flex items-center gap-2 text-slate-500 mb-1">
                                        <Tag size={14} />
                                    ...
                                    <p className="text-sm font-bold text-slate-800 dark:text-slate-200 uppercase">{selectedDeviceForInfo.warehouse}</p>
                                </div>
                            </div>
                                ...
                                <div>
                                    ...
                                    {isEditingModal ? (
                                        ...
                                        <div className="bg-slate-50 dark:bg-slate-950/50 p-4 rounded-xl border border-slate-200 dark:border-slate-800 min-h-[48px] flex items-center">
                                            {selectedDeviceForInfo.ticket ? (
                                                <span className="text-sm font-mono font-bold text-blue-500">#{selectedDeviceForInfo.ticket}</span>
                                            ) : (
                                ...
                                <div>
                                    ...
                                    {isEditingModal ? (
                                        ...
                                        <div className="bg-slate-50 dark:bg-slate-950/50 p-4 rounded-xl border border-slate-200 dark:border-slate-800 min-h-[100px]">
                                            {selectedDeviceForInfo.observation ? (
                                                <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed font-medium">{selectedDeviceForInfo.observation}</p>
                                            ) : (
                            ...
                            {isEditingModal ? (
                                <>
                                    <button 
                                        ...
                                        disabled={savingId === selectedDeviceForInfo.uuid}
                                    ...
                                    >
                                        {savingId === selectedDeviceForInfo.uuid ? <Loader2 size={18} className="animate-spin" /> : <FileText size={18} />}
                                        Salvar Alterações
                                    </button>
                                    <button onClick={() => handleResolveIssue(selectedDeviceForInfo.uuid)} className="flex-1 bg-emerald-600/10 hover:bg-emerald-600 text-emerald-600 hover:text-white border border-emerald-600/20 font-black py-3 rounded-xl transition-all flex items-center justify-center gap-2 uppercase text-xs tracking-widest">
                                        <CheckCircle2 size={18} />
                                        Marcar como Online
                                    </button>
                                </>
                            ) : (
                                <button onClick={() => handleResolveIssue(selectedDeviceForInfo.uuid)} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-black py-3 rounded-xl transition-all flex items-center justify-center gap-2 uppercase text-xs tracking-widest shadow-lg shadow-emerald-500/20">
                                    <CheckCircle2 size={18} />
                                    Resolver Incidente
                                </button>
                            )}
